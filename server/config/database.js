...
class DatabaseManager {
  ...
  async createSystemTables() {
    try {
      ...
      await query(`
        CREATE TABLE IF NOT EXISTS autres.permissions (
          id INT AUTO_INCREMENT PRIMARY KEY,
      ...
      await query(`
        CREATE TABLE IF NOT EXISTS autres.roles (
          id INT AUTO_INCREMENT PRIMARY KEY,
      ...
      await query(`
        CREATE TABLE IF NOT EXISTS autres.role_permissions (
          role_id INT NOT NULL,
          permission_id INT NOT NULL,
          PRIMARY KEY (role_id, permission_id),
          CONSTRAINT fk_role_permissions_role FOREIGN KEY (role_id) REFERENCES autres.roles(id) ON DELETE CASCADE,
          CONSTRAINT fk_role_permissions_permission FOREIGN KEY (permission_id) REFERENCES autres.permissions(id) ON DELETE CASCADE
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
      `);

      await query(`
        CREATE TABLE IF NOT EXISTS autres.user_roles (
          user_id INT NOT NULL,
          role_id INT NOT NULL,
          PRIMARY KEY (user_id, role_id),
          INDEX idx_user_roles_role (role_id),
          CONSTRAINT fk_user_roles_user FOREIGN KEY (user_id) REFERENCES autres.users(id) ON DELETE CASCADE,
          CONSTRAINT fk_user_roles_role FOREIGN KEY (role_id) REFERENCES autres.roles(id) ON DELETE CASCADE
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
      `);
      ...
      await query(`
        CREATE TABLE IF NOT EXISTS autres.upload_history (
          ...
          INDEX idx_upload_history_job (job_id),
          CONSTRAINT fk_upload_history_user FOREIGN KEY (user_id) REFERENCES autres.users(id) ON DELETE SET NULL
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
      `);
      ...
      const ensureUploadIndex = async (indexName, definition) => {
        try {
          await this.pool.execute(`ALTER TABLE autres.upload_history ADD INDEX ${indexName} ${definition}`);
        } catch (error) {
          if (error?.code !== 'ER_DUP_KEYNAME') {
            throw error;
          }
      ...
      await ensureUploadIndex('idx_upload_history_status', '(status)');
      await ensureUploadIndex('idx_upload_history_job', '(job_id)');
