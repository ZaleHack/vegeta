...
router.get('/realtime/export', authenticate, async (req, res) => {
  try {
    ...
    const worksheetData = rows.map((row) => ({
      type_appel: row.type ?? null,
      date_debut: row.callDate ?? null,
      heure_debut: row.startTime ?? null,
      duree_sec: row.duration ?? null,
      date_fin: row.endDate ?? null,
      heure_fin: row.endTime ?? null,
      numero_appelant: row.caller ?? null,
      numero_appele: row.callee ?? null,
      imsi_appelant: row.imsiCaller ?? null,
      imei_appelant: row.imeiCaller ?? null,
      cgi: row.cgi ?? null,
      route_reseau: row.networkRoute ?? null,
