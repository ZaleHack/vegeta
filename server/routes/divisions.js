...
router.post('/', authenticate, requirePermission('admin:manage_divisions'), async (req, res) => {
  try {
    const { name } = req.body;
...
router.delete('/:id', authenticate, requirePermission('admin:manage_divisions'), async (req, res) => {
  try {
...
router.get('/:id/users', authenticate, async (req, res) => {
  try {
    ...
    const hasDivisionPermission = Array.isArray(req.user?.permissions)
      ? req.user.permissions.includes('admin:manage_divisions')
      : false;
    const isAdmin = req.user?.admin === 1 || req.user?.admin === '1' || req.user?.admin === true;

    if (!hasDivisionPermission && !isAdmin && req.user?.division_id !== divisionId) {
