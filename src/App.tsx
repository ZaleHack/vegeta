            ...
            <div key={index} className="bg-white/90 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-slate-200 hover:shadow-xl transition-all duration-200">
              <div className="mb-4">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  {result.score > 0 && (
                    { query: 'nom:Dupont AND prenom:Jean', desc: 'Recherche ET', tip: 'Tapez "nom:" puis sélectionnez dans la liste' },
                    { query: 'telephone:77 OR telephone:76', desc: 'Recherche OU', tip: 'Autocomplétion disponible pour "telephone"' },
                    { query: 'corps:police NOT grade:gardien', desc: 'Avec exclusion', tip: 'Les champs "corps" et "grade" sont suggérés' },
                    { query: '"Ministère des Finances"', desc: 'Expression exacte', tip: 'Utilisez les guillemets pour les expressions' },
                    { query: 'region:Dakar AND sexe:M', desc: 'Critères multiples', tip: 'Tapez les premières lettres du champ' },
                    { query: 'matricule:123 AND NOT ville:Pikine', desc: 'Combinaison complexe', tip: 'Autocomplétion pour tous les champs' }
                    .map(([key, value]) => (
                    <div key={key} className="bg-slate-50 rounded-lg p-3">
                      <span className="text-xs text-slate-500 uppercase tracking-wide font-medium block mb-1">
                        {key.replace(/_/g, ' ')}
                      className="p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-blue-50 transition-colors border border-gray-200"
                      title={example.tip}
                ...
                {/* Guide d'utilisation de l'autocomplétion */}
                <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <h4 className="font-semibold text-blue-800 mb-2">🚀 Autocomplétion des champs</h4>
                  <div className="text-sm text-blue-700 space-y-1">
                    <div>• <strong>Tapez les premières lettres</strong> d'un nom de champ (ex: "nom", "tel", "mat")</div>
                    <div>• <strong>Sélectionnez dans la liste</strong> qui apparaît automatiquement</div>
                    <div>• <strong>Le ":" est ajouté automatiquement</strong> après sélection</div>
                    <div>• <strong>Plus de 50 champs disponibles</strong> : nom, telephone, matricule, CNI, etc.</div>
                  </div>
                </div>
              </div>
            </div>
